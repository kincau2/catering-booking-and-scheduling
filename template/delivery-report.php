...
$records = $wpdb->get_results( $wpdb->prepare(
    "SELECT * FROM {$ct} WHERE date BETWEEN %s AND %s",
    $start, $end
) );
...
// Column titles
$sheet1->setCellValue('A4','Category');
$sheet1->setCellValue('B4','Meal');
$sheet1->setCellValue('C4','Quantity');
...
foreach ( $records as $rec ) {
    ...
    if( isset($addr['delivery_note']) && !empty($addr['delivery_note']) ){
        $delivery_note[] = _('Delivery Note' ) . ': '. $addr['delivery_note'];
    }

    if ( ! empty( $WC_item->get_meta('cs_note', true) ) ) {
        $delivery_note[] = _('CS Note' ) . ': '. $WC_item->get_meta('cs_note', true);
    }
    ...
    if ( is_array( $health_status['allergy'] ) && ! empty( $health_status['allergy'] ) ) {
        ...
        if ( ! empty( $health_status['allergy'] ) ) {
            $delivery_note[] = _('Food allergy' ) . ': ' . implode( ', ', $health_status['allergy'] );
        }
    }
...
foreach ( $records as $rec ) {
    ...
    if ( ! empty( $addr['delivery_note'] ) ) {
        $delivery_note[] = __('Delivery Note') . ': ' . $addr['delivery_note'];
    }
    if ( ! empty( $WC_item->get_meta('cs_note', true) ) ) {
        $delivery_note[] = __('CS Note') . ': ' . $WC_item->get_meta('cs_note', true);
    }
    $health_status = maybe_unserialize( $booking->health_status );
    if ( is_array( $health_status['allergy'] ) && ! empty( $health_status['allergy'] ) ) {
        $health_status['allergy'] = array_diff( $health_status['allergy'], ['no_allergy'] );
        if ( ! empty( $health_status['allergy'] ) ) {
            $delivery_note[] = __('Food allergy') . ': ' . implode(', ', $health_status['allergy']);
        }
    }
