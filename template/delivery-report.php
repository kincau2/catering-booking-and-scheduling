...
usort($other_cids, function($a, $b) use ($term_ordering) {
    $orderA = $term_ordering[$a] ?? 999999;
    $orderB = $term_ordering[$b] ?? 999999;
    if ($orderA === $orderB) {
        return $a - $b; // fallback to ID if ordering is the same
...
foreach ( $ordered_categories as $cat_id ) {
    ...
    // Add border around this category group
    $row_end = $rowNum - 1;
    $sheet1->getStyle("A{$row_start}:C{$row_end}")
            ->getBorders()
            ->getOutline()
...
if ( $group3 ) {
    ...
    if ( $container_counts ) {
        // Store soup section start row for border
        $soup_start_row = $rowNum;
        ...
        // Add border around soup category group
        $soup_end_row = $rowNum - 1;
        $sheet1->getStyle("A{$soup_start_row}:C{$soup_end_row}")
                ->getBorders()
                ->getOutline()
                ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
